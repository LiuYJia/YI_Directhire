...
export class RecuserquePage {
  ...
  sub(){
    if(this.txt == null){
      ...
      this.mes = '';
      this.http.post('http://127.0.0.1:3000/user/updateRecruit/feedback',JSON.stringify({username:localStorage.getItem("login"),sort:this.sort,txt:this.txt,tel:this.tel}),{headers:this.headers}).subscribe(data=>{
